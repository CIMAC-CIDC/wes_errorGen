...
def get_downstream(node, nodes):
    '''takes a job ID and identifies downstream job IDs'''
    downstream = [i for i in nodes[node]]
    downstream.append(node) # MOVE THIS LATER BUT WE NEED TO HANDLE BROKEN RULE TOO!
    queue = [i for i in nodes[node]]
    while queue: # checks if empty
        n = queue.pop()
        downstream.append(n)# we will get unique values later
        if n in nodes: # some rules are endpoints
            children=nodes[n]
...
def get_node_files(node, graph, files):
    ...
    #print(type("label"))
    #print(files[files.rule==label])
    f=files.loc[files['rule'] == label, ['output_file']] # replace with f["output_file"].filter()?
...
def main():
    ...
    graph=graph[0]
    #print(graph)
    #print(help(pydot.Node))
    #print(help(pydot.Graph))
    ...
    # with open(args.yaml, "r") as config_file: # perhaps we should use metashhet.csv instead?
    #     try:
    ...
    for file in affected_files:
        ...
        #add condition here to handle unhealthy looking file
            ...
            #file_status="uncertain" # find better word
        ...
        valid = False
        while not valid:
            comment = input("Enter a comment or press 'Enter' to skip: ")
            if comment == '':
                valid =True
            else:
